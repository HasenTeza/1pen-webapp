...
Page({
  ...
  onLoad: function (options) {
    console.log(options,'这是普通的地址')
    var that = this
    var from = options.from
    // console.log(options, options.from, options.fromz, 'from 跳转')
    that.setData({
      from: from,
      cart: options.cart
    })
        ...
        success: function (res) {
          ...
          if (res.data.result == 1) {
            wx.request({
              ...
              data: { openid: app.globalData.openid, bis_id: app.globalData.bis_id, cart:that.data.cart},
              method: 'post',
              header: {
                'content-type': ''
              },
              success: function (cartResult) {
                console.log(cartResult, '选中后带数据返回')
